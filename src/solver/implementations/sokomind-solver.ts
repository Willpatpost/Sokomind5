...
const STRUCTURAL_HEAD_START_MS = 25_000;
...
function walkBetween(
  ...
  const paths = new Map<string, LegacyDirection[]>([[startKey, []]]);
  ...
  for (let head = 0; head < queue.length; head += 1) {
    ...
    for (const direction of DIRECTION_VECTORS) {
      ...
      paths.set(nextKey, [...path, direction.legacy]);
...
function structuralHeadStartMs(run: SearchRunState): number {
  ...
  return Math.min(
    ...
    remaining * STRUCTURAL_TIME_SHARE,
  );
}
    ...
    availableExpanded === undefined
      ...
      : Math.max(
          ...
          Math.min(
            availableExpanded - 1,
            Math.floor(
              availableExpanded * STRUCTURAL_STATE_SHARE,
            ),
          ),
        );
    ...
    availableGenerated === undefined
      ...
      : Math.max(
          ...
          Math.min(
            ...
            Math.floor(
              availableGenerated * STRUCTURAL_STATE_SHARE,
            ),
          ),
        );
  return Object.freeze({
      ...
      if (structural && !stopReason) {
        ...
        if (!remainingRequest) {
          ...
          const structuralRequest =
            withStructuralStateBudget(remainingRequest);
