...
export function hasFreezeDeadlock(
  ...
  while (changed) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Horizontal axis: left (2) and right (3)
  ...
  // Deadlock if any frozen box is not on its matching goal
  for (let i = 0; i < n; i++) {
    if (frozen[i] && board.goalLabelByCell[boxes[i].cell] !== boxes[i].label) {
      return true;
    }
  }
  return false;
