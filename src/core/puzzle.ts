...
import { freezePosition } from "./position.ts";
...
export function parsePuzzleRows(rows: readonly string[]): ParsedBoard {
  const validation = validatePuzzleRows(rows);
  if (!validation.valid) throw new PuzzleValidationError(validation.errors);
  ...
  // Validation above guarantees this assignment.
  if (!initialRobot) {
...
export function parsePuzzle(puzzle: PuzzleDefinition): ParsedBoard {
  ...
  if (!validation.valid) throw new PuzzleValidationError(validation.errors);
  return parsePuzzleRows(puzzle.rows);
}
